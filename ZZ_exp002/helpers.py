...
def generate_mask(dic, mask_size, default_size=-1):
    """
    dic: 分子名(target)をキーとし、座標のリストを値とした辞書
    mask_size: 分子名をキー、半径(浮動小数も可)を値とする辞書
    default_size: 全ての分子に同じ半径を適用したい場合に使う(-1のときは使用しない)

    戻り値:
        mask: shape=(Z, X, Y)=(184, 630, 630)の3次元配列
              ボクセルが属する分子のIDが格納されたマスク
    """

    # default_size が指定されていれば、すべての分子に同じサイズを適用
    if default_size != -1:
        mask_size = {key: default_size for key in MOLECULES}
    ...
    for target in MOLECULES:
        ...
        # 浮動小数点の半径(r)をカバーするため、天井(ceil)で囲む整数値の範囲を決定
        # 例: radius = 2.5 の場合 → int_rad = 3
        int_rad = math.ceil(radius)

        for coord in dic[target]:
            # 座標の取得 (dict中の順序に注意: y, x, z になっている場合など)
            y, x, z = coord

            # スケーリング(1/10)して float として扱う
            x, y, z = x / 10.0, y / 10.0, z / 10.0

            # -int_rad ～ +int_rad までループし、球状の範囲を塗りつぶす
            for dz in range(-int_rad, int_rad + 1):
                for dx in range(-int_rad, int_rad + 1):
                    for dy in range(-int_rad, int_rad + 1):
                        # 球内にあるか (floatでチェック)
                        if (dx**2 + dy**2 + dz**2) <= radius**2:
                            ...
                            if (0 <= nx < mask.shape[1] and
                                0 <= ny < mask.shape[2] and
                                0 <= nz < mask.shape[0]):

                                # インデックスとして使うために int に変換
                                nx, ny, nz = int(nx), int(ny), int(nz)
                                if mask[nz, nx, ny] != 0:
                                    #print(f"Warning: Overlapping mask at {nx}, {ny}, {nz}, ID={ID},before={mask[nz, nx, ny]}")
                                    #mask[nz, nx, ny] = ID
                                    continue
                                else :
                                    mask[nz, nx, ny] = ID
    ...
    for i, id in enumerate(test_id):
        ...
        for z in zz:
            for seg_x, seg_y in [(i % 3, i // 3) for i in range(9)]:
                ...
                image = meta_volume[z:z + num_slice, seg_x*187:seg_x*187+256, seg_y*187:seg_y*187+256]
                mask   = meta_mask[z:z + num_slice, seg_x*187:seg_x*187+256, seg_y*187:seg_y*187+256]
                ...
                prob = output['particle'][0].cpu().numpy()
                probability[:, z:z + num_slice, seg_x*187:seg_x*187+256, seg_y*187:seg_y*187+256] += prob[:, :, :,:]
                count[:, z:z + num_slice, seg_x*187:seg_x*187+256, seg_y*187:seg_y*187+256] += 1
        probability = probability / (count + 0.0001)
        location = probability_to_location(probability, cfg)
        df = location_to_df(location)
        df.insert(loc=0, column='experiment', value=id)
