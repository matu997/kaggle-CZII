...
def dice_loss(pred, target, smooth=1e-6):
    ...
    Args:
        pred (torch.Tensor): モデルの出力 (B, C, D, H, W) - 各クラスの確率スコア。
        target (torch.Tensor): ターゲットマスク (B, D, H, W) - 各ボクセルのクラスインデックス。
        smooth (float): 数値安定性を向上させるためのスムージング項。

    Returns:
        torch.Tensor: Dice Loss の値。
    """
    # 予測の形状 (B, C, D, H, W)
    num_classes = pred.size(1)
    
    # ターゲットマスクを One-hot エンコード (B, D, H, W) -> (B, D, H, W, C)
    target_one_hot = F.one_hot(target, num_classes=num_classes).permute(0, 4, 1, 2, 3).float()  # (B, C, D, H, W)

    # ソフトマックスで確率マップに変換 (B, C, D, H, W)
    pred = F.softmax(pred, dim=1)

    # 各クラスごとの Dice Score を計算
    intersection = (pred * target_one_hot).sum(dim=(2, 3, 4))  # (B, C)
    union = pred.sum(dim=(2, 3, 4)) + target_one_hot.sum(dim=(2, 3, 4))  # (B, C)
...
class Net(nn.Module):
    ...
    def forward(self, batch):
        ...
        if 'loss' in self.output_type:
            logit = self.mask(last)
            dice = dice_loss(logit, batch['mask'].to(device))
            output['mask_loss'] = dice
